...
if $opt_build; then
    ...
    # Overwrite setup.py with a version that doesn't try to build the C code
    rm -f "${g_output_dir}/src/setup.py"
    echo "#! /usr/bin/env python" >> "${g_output_dir}/src/setup.py"
    echo "" >> "${g_output_dir}/src/setup.py"
    echo "from distutils.command.build import build as _build" >> "${g_output_dir}/src/setup.py"
    echo "from setuptools import setup" >> "${g_output_dir}/src/setup.py"
    echo "" >> "${g_output_dir}/src/setup.py"
    echo "class build(_build):" >> "${g_output_dir}/src/setup.py"
    echo "    @staticmethod" >> "${g_output_dir}/src/setup.py"
    sed -ne '/^[[:space:]]*def pythonBuildDirectory/,/^[[:space:]]*$/p' "${g_srcroot_dir}/setup.py" >> "${g_output_dir}/src/setup.py"
    sed -ne '/^setup(/,$p'  "${g_srcroot_dir}/setup.py" >> "${g_output_dir}/src/setup.py"
