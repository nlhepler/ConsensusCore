...
namespace detail {
    ...
    // and: http://yosefk.com/c++fqa/templates.html#fqa-35.16  (for hilarious rejoinder)
    template<typename T> class VectorL;  // fwd
    template<typename T> T Max(const VectorL<T>& v);
    template<typename T> size_t ArgMax(const VectorL<T>& v);

    //
    // Vector class that stores only a subsequence of the rows
    // (SparseVector has a lot of weird functionality built-in, so we can't use it without
    ...
    {
        return v.beginRow_ + distance(v.storage_.begin(), max_element(v.storage_.begin(), v.storage_.end()));
    }
